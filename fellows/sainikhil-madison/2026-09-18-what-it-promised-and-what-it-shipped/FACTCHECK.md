...
- [x] B04 landscape (3840×2160) — 15% (row 1), 50% (rows 1–2), 85% (rows 1–3)
- [x] B04 portrait (2160×3840) — same three states, `TypesetMath916`
- [x] Fraction bars unbroken; `TP`/`FP`/`FN` upright and unclipped; `F_1`
      subscript not colliding with the fraction; no glyph fallback boxes
- [x] Readable at ordinary viewing size, not only at 4K
- [x] Staged reveal verified: at 50% only P and R are on screen, so the
      "differ by one term" claim is visually checkable — FP becomes FN and
      nothing else moves. Row 3 lands after.

**Reviewed 2026-09-18 on the rendered beats in both aspects. No defect found;
no algebra correction required.** GATE V on both CLEAN masters:
`frames=18 BLOCKER=0 MAJOR=0`.

> Note on GATE V and the 9:16: the first portrait pass reported 18/18
> edge-bleed blockers. Those were entirely the SLATE's burned-in review
> markers (timecode top, `B## <act>` bottom), not content. Re-running the gate
> against the clean master returns 0/0. Always audit the clean master.
