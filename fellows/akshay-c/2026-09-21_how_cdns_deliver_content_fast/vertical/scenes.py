...
class B03_Partitions(BrutalistScene):
    def construct(self):
        ...
        router.move_to(UP * 3.7)

        edges = VGroup(
            self.node("EDGE A"),
            self.node("EDGE B", GOOD),
            self.node("EDGE C")
        ).arrange(DOWN, buff=.55).move_to(DOWN * .4)
...
class B10_Replication(BrutalistScene):
    def construct(self):
        ...
        states.arrange(DOWN, buff=.75).move_to(UP * .4)

        arrows = VGroup(
            Arrow(states[0].get_bottom(), states[1].get_top(), buff=.15, color=MUTED),
            Arrow(states[1].get_bottom(), states[2].get_top(), buff=.15, color=ACCENT)
        )
...
class B11_FullScale(BrutalistScene):
    def construct(self):
        ...
        origin.move_to(UP * 3.8)

        edges = VGroup(
            self.node("EDGE · AMERICAS"),
            self.node("EDGE · EUROPE"),
            self.node("EDGE · ASIA")
        ).arrange(DOWN, buff=.55).move_to(DOWN * .25)
...
class B12_Tradeoffs(BrutalistScene):
    def construct(self):
        ...
        cards.arrange(DOWN, buff=.48).move_to(UP * .25)
