...
def draw_spectrum(d, c, U):
    """A general↔specialized axis with N labeled markers placed left→right."""
    W, H, port = U["W"], U["H"], U["port"]
    markers = c["markers"]; ll = c.get("left_label", "general"); rl = c.get("right_label", "specialized")
    acc = c.get("accent_index", -1)
    n = len(markers)
    if not port:
        y = 0.52 * H; x0 = 0.15 * W; x1 = 0.85 * W
        arrow(d, x0, y, x1, y, INK_SOFT, U["aw"], U["ah"])
        xs = [x0 + (x1 - x0) * (i + 0.5) / n for i in range(n)]
        for i, (x, m) in enumerate(zip(xs, markers)):
            col = SPARK if i == acc else INK
            r = int(0.011 * W)
            d.ellipse([x - r, y - r, x + r, y + r], fill=col)
            center(d, x, y - 0.10 * H, m["label"], U["f_hub"], col)
            if m.get("sub"):
                for j, ln in enumerate(wrap(d, m["sub"], U["f_small"], 0.19 * W)):
                    center(d, x, y + 0.055 * H + j * int(0.024 * W), ln, U["f_small"], INK_SOFT)
        d.text((x0, y + 0.155 * H), ll.upper(), font=U["f_foot"], fill=INK_SOFT, anchor="lm")
        d.text((x1, y + 0.155 * H), rl.upper(), font=U["f_foot"], fill=INK_SOFT, anchor="rm")
    else:
        x = 0.28 * W; y0 = 0.30 * H; y1 = 0.74 * H
        arrow(d, x, y0, x, y1, INK_SOFT, U["aw"], U["ah"])
        ys = [y0 + (y1 - y0) * (i + 0.5) / n for i in range(n)]
        for i, (yy, m) in enumerate(zip(ys, markers)):
            col = SPARK if i == acc else INK
            r = int(0.011 * W)
            d.ellipse([x - r, yy - r, x + r, yy + r], fill=col)
            d.text((x + 0.055 * W, yy - 0.012 * H), m["label"], font=U["f_hub"], fill=col, anchor="lm")
            if m.get("sub"):
                d.text((x + 0.055 * W, yy + 0.022 * H), m["sub"], font=U["f_small"], fill=INK_SOFT, anchor="lm")
        center(d, x, y0 - 0.03 * H, ll, U["f_note"], INK_SOFT)
        center(d, x, y1 + 0.03 * H, rl, U["f_note"], INK_SOFT)

KINDS = {"flow": draw_flow, "grid": draw_grid, "example": draw_example,
         "versus": draw_versus, "loop": draw_loop, "spectrum": draw_spectrum}
